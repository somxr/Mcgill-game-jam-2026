...
func check_transition(input: InputPackage) -> String:
	...
	if input.actions[0] == "walk" or player.velocity != Vector3.ZERO:
		return "unchanged"
	return input.actions[0]
	
func update(input: InputPackage, delta: float):
	calculate_velocity_from_input(input, delta)
...
func calculate_velocity_from_input(input: InputPackage, delta: float):
	# Separating XZ from Y velocity so we can manipulate them separately.
	# The player only controls movement in the XZ plane while jumping and gravity affects Y.
	var current_velocity_xz := Vector2(player.velocity.x, player.velocity.z)

	# Get the direction the visual mesh is looking towards in the XZ plane
	var facing_direction := Vector2(visual.basis.z.x, visual.basis.z.z)

	# Gives you the angle IN RADIANS from the visual's facingDirection vector to the input vector
	angle = facing_direction.angle_to(Vector2(input.input_direction.x, input.input_direction.y))
	...
	if input.input_direction:
		...
		if abs(angle) >= current_angular_speed * delta:
			# Rotate the velocity vector by angular speed each frame. So the turning is gradual.
			# Sign(angle) just decides if clockwise or anticlockwise
			...
			current_velocity_xz = facing_direction.rotated(sign(angle) * current_angular_speed * delta) * current_speed
		else:
			...
			current_velocity_xz = facing_direction.rotated(angle) * current_speed
	else:
		...
		current_velocity_xz = current_velocity_xz.move_toward(Vector2.ZERO, DECELERATION * delta)

	# Final velocity is determined based on the branching of the if statements,
	# whatever it is gets assigned as the final player velocity here
	player.velocity = Vector3(current_velocity_xz.x, player.velocity.y, current_velocity_xz.y)
